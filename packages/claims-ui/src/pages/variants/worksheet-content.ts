...
export const DEATH_DECISION_OPTIONS = [
  {
    ...
    description:
      'All checks pass. DCI from HORD to settlement. Route to Payment Process. TPA authority up to $100K aggregate.',
  },
  {
    id: 'approve-suicide',
    title: 'Approve — Pay suicide provision (D-25)',
    description:
      'Suicide confirmed within contestable period. Payment = return of premiums paid to DOD (not face value).',
  },
  {
    id: 'deny-tpa',
    title: 'Deny — within TPA authority (D-26)',
    description:
      'Very limited scenarios. Appeal language for CA IL NE NH NJ RI TN WA WV. Most denials require Pru first.',
  },
  {
    id: 'refer-pru',
...
export const TI_DECISION_OPTIONS = [
  {
    ...
    description:
      'Medical expert approves. Face × % up to 95%, minus $100 admin fee. DCI HORD to settlement. Policy terminates on payment. TPA: $50K TI / $101K aggregate.',
  },
  {
    id: 'deny-ti-life',
    title: 'Deny — Life expectancy (T-26)',
    description:
      'LE >6 months (>12 months CA). TPA denial authority — does not require Pru approval.',
  },
  {
    id: 'deny-ti-other',
...
export function decisionOptionsForProduct(product: ClaimProduct) {
  return product === 'ti' ? TI_DECISION_OPTIONS : DEATH_DECISION_OPTIONS
}

export function renderDeathExaminerReview(): TemplateResult {
  return html`
...
export function renderTiExaminerReview(): TemplateResult {
  return html`
    <div class="grid grid-cols-[1fr_220px] gap-2.5">
      <div>
        <div class="flex items-center justify-between mb-1.5">
...
export function renderDeathClaimTools(): TemplateResult {
  return html`
...
export function renderTiClaimTools(): TemplateResult {
  return html`
    <div class="grid grid-cols-1 md:grid-cols-2 gap-2.5">
...
export function renderSystemAssessment(product: ClaimProduct): TemplateResult {
  if (product === 'ti') {
    return html`
